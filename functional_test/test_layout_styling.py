...
class ProductListLayoutStylingTest(FunctionalTest):
    ...
    def test_can_see_all_products_list_and_details_links(self):
        ...
        products_link = self.browser.find_element_by_link_text('All products')
        products_link.click()
        self.wait_for(lambda:
                      self.assertIn(self.browser.find_element_by_tag_name('h1').text, 'All products'))

        # User can see all products as pictures in rows
        all_products = self.browser.find_elements_by_id('product')
        self.assertEqual(len(Product.objects.all()), all_products)

        # User clicks on a picture to see available actions - add to basket or see details
        product = self.browser.find_element_by_id('product')
        product.click()
        self.assertTrue(self.browser.find_element_by_name('add-to-basket'))
        self.assertTrue(self.browser.find_element_by_link_text('See details'))
...
class ProductDetailLayoutStylingTest(FunctionalTest):
    ...
    def test_can_see_product_details(self):
        ...
        product_element = self.browser.find_element_by_id('product')
        ...
        self.assertEqual(product_id, details_link.split('/')[-1])
