...
class AddProductToBasketTest(FunctionalTest):
    ...
    def test_can_add_product_to_basket_from_detail_page(self):
        ...
        self.assertEqual(self.browser.find_element_by_tag_name('h3').text, 'You have 1 item in your cart')
    ...
    def test_can_update_product_quantity_in_basket(self):
        ...
        self.browser.find_element_by_class_name('plus-one').click()
        self.assertEqual(self.browser.find_element_by_tag_name('h3').text, 'You have 2 items in your cart')

        # they click minus and 1 is subtracted form this product quantity
        self.browser.find_element_by_class_name('minus-one').click()
        self.assertEqual(self.browser.find_element_by_tag_name('h3').text, 'You have 1 item in your cart')
    ...
    def test_navbar_qunantity_updates(self):
        # Cart items quantity is updated in navbar after adding a product to cart
        self.browser.get(self.live_server_url)
        self.assertEqual(0, int(self.browser.find_element_by_id('nav-cart-items').text))
        tests_helpers.add_items_to_cart_ft(self)
